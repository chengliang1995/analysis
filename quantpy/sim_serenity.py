"""模拟盘 Serenity 卡脖子子账户（独立 20 万）。

与超短 / 三倍量中线 / MA20 回踩账户隔离，state key = ``serenity``。
...
DEFAULT_SERENITY_CAPITAL = 200_000.0


@dataclass
class SerenitySimConfig:
    capital: float = DEFAULT_SERENITY_CAPITAL
    max_positions: int = 5
    ...
    # 均线空头则离场（与卡脖子中线持有纪律对齐）
    exit_on_ma_bear: bool = True


def default_serenity_state() -> dict:
    cfg = SerenitySimConfig()
    return {
        "config": asdict(cfg),
...
def ensure_serenity_state(state: dict) -> dict:
    if "serenity" not in state or not isinstance(state.get("serenity"), dict):
        state["serenity"] = default_serenity_state()
        return state["serenity"]
    mt = state["serenity"]
    defaults = default_serenity_state()
    for key, val in defaults.items():
        if key not in mt:
            mt[key] = val
    if "config" not in mt:
        mt["config"] = defaults["config"]
    # 固定初始资金口径 20 万（仅当账户仍为空仓且现金≈旧初始时不强制改；新账户用默认）
    cfg = mt.get("config") or {}
    if "capital" not in cfg:
        cfg["capital"] = DEFAULT_SERENITY_CAPITAL
        mt["config"] = cfg
    return mt


def _cfg(mt: dict) -> SerenitySimConfig:
    return SerenitySimConfig(**{**asdict(SerenitySimConfig()), **(mt.get("config") or {})})


def _ma_label(code: str) -> str:
    try:
        hist = get_stock_hist(code, days=90, patch_live=False)
    except Exception:
        return "unknown"
...
def _serenity_held(engine: "SimReplayEngine") -> set[str]:
    mt = ensure_serenity_state(engine.state)
    held = {str(p["code"]).zfill(6) for p in mt.get("positions", [])}
    held |= _sim_held_codes(engine)
    # 不与 midterm / ma20 强互斥：仅避免超短主账户同码重复；中线可并行观察
    return held


def check_serenity_sim_exits(
    ...
    *,
    ...
    for p in positions:
        ...
        elif cfg.exit_on_ma_bear and _ma_label(code) == "空头":
            sell_price = price
            reason = "均线转空头离场"
        ...
        _progress(
            f"  [Serenity模拟] 卖出 {p['name']}({code}) @{sell_price:.2f} {reason}",
            show_progress,
        )
    ...
    *,
    ...
    for rec in recs:
        ...
        pos = {
            ...
            "score": score,
            "theme": theme or rec.get("board_name") or "",
            "reason": f"Serenity卡脖子 · {theme or rec.get('board_name') or ''}",
            "tags": tags_s,
            "ma_structure": rec.get("ma_structure"),
            "strategy": STRATEGY_ID,
        ...
        new_count += 1
        _progress(
            f"  [Serenity模拟] 买入 {pos['name']}({code}) {qty}股 @{buy_price:.2f} 分{score:.0f}",
            show_progress,
        )
...
def record_serenity_picks(
    ...
    *,
    ...
    if logged:
        ...
        mt["last_scan"] = [
            {
                ...
                "score": e["score"],
                "price": e["price"],
                "ma_structure": e.get("ma_structure"),
                "board_name": e.get("board_name"),
                "tags": ",".join(e["tags"]) if isinstance(e.get("tags"), list) else (e.get("tags") or ""),
            }
            for e in logged
        ]
        if theme:
        ...
        mt["updated_at"] = datetime.now().isoformat()
        engine._save_state()
        _progress(f"  [Serenity模拟] 记录选股 {len(logged)} 只", show_progress)
    return logged


def run_sim_serenity_select(
    engine: "SimReplayEngine",
    *,
    theme: str = "",
    ...
    try:
        ...
        held = _serenity_held(engine)
        scan = run_serenity_scan(
            theme,
            board_type=board_type,
            board_code=board_code,
            top_boards=3,
            max_candidates=max_candidates,
            record_picks=False,  # 模拟建仓不污染实盘跟进文件
            show_progress=show_progress,
        )
        if not scan.get("ok"):
            return {
                "ok": False,
                "message": scan.get("message") or "Serenity 扫描失败",
                "summary": enrich_serenity_sim(engine.state),
            }
        ...
        closed = check_serenity_sim_exits(engine, show_progress=show_progress)
        logged = record_serenity_picks(engine, recs, theme=theme, show_progress=show_progress)
        buy_result = run_serenity_sim_buy(
            engine, recs, theme=theme, show_progress=show_progress, force=force,
        )
        buy_n = len(buy_result.get("bought", []))
        message = (
            f"Serenity 命中 {len(recs)} 只，买入 {buy_n} 只（主题 {theme or board_code}）"
            if recs
            else f"主题「{theme or board_code}」暂无合格候选"
        )
        ...
        return {
            "ok": False,
            "message": f"Serenity 模拟选股失败: {exc}",
            "error": err,
            "summary": enrich_serenity_sim(engine.state),
        }


def enrich_serenity_sim(state: dict, quotes_df: Optional["pd.DataFrame"] = None) -> dict:
