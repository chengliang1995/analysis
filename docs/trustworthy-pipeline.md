...
| 中线模拟买入 | 仅 `triple_volume_watchlist.get_buy_signal_recommendations(today_only=True)` + 当日价 |
...
- 模拟平仓收益扣除 `round_trip_cost_pct`（默认 0.12%），避免虚高胜率。
- 持仓天数：日记与 AI 样本优先**交易日**间隔（`trade_journal.hold_days`、`_load_real_ultra_trades`）。
