...
Write-Host "  powershell -ExecutionPolicy Bypass -File `"$Runner`" -Phase ma20-am"
Write-Host "  powershell -ExecutionPolicy Bypass -File `"$Runner`" -Phase triple-volume"
Write-Host "  morning sim order: sim-short-term -> sim-serenity -> sim -> sim-midterm-select"
Write-Host ""
$removeScript = Join-Path $ProjectRoot "scripts\remove_scheduled_tasks.ps1"
Write-Host "删除全部: powershell -ExecutionPolicy Bypass -File `"$removeScript`""
